...
public static class DATReportCreator
{
	...
	/// <param name="widerColumn"></param>
	/// <param name="width"></param>
	/// <returns></returns>
	public static Document createSimpleReport<T>(List<T> objects,String reportTitle, String company, String software, string widerColumn = "", int width = 0, bool landscape = false)
	{
		uint currentRow = 1;
		var document = Document.Create(container =>
		{
			container.Page(page =>
			{
				setTemplate(ref page,reportTitle,company,software,landscape);
				...
				.Column(column =>
				{
					...
					{
						currentRow = createSimpleTable(table, objects, currentRow,widerColumn,width);
					});
				});
			});
		});
	...
	public static Document createSingleGroupingReport<T>(List<T> objects, String groupBy, String reportTitle, String company, String software, List<String> filterOrder = null, string widerColumn = "", int width = 0,bool landscape = false)
	{
		uint currentRow = 1;
		var document = Document.Create(container =>
		{
				...
				.Column(column =>
				{
					column.Item().Table(table =>
					{
						...
						foreach (var item in set)
						{
							...
							currentRow++;
							currentRow = createSimpleTable(table, objects.Where(i => i.GetType().GetProperty(groupBy).GetValue(i) == item).ToList(), currentRow,widerColumn,width, 1);
						}
					});
				});
			});
	...
	private static void setTemplate(ref PageDescriptor page,String reportTitle, String company, String software,bool landscape = false)
	{
		if (landscape)
	...
	/// <param name="objects"></param>
	private static uint createSimpleTable<T>(TableDescriptor table,List<T> objects, uint currentRow, string widerColumn, int width, uint groupingColumnsn = 0)
	{
		//Does nothing if the list is empty
		uint currentColumn = 1;
		if (objects == null || objects.Count == 0) return currentRow;
		currentRow = printHeaders(table, objects, currentRow, groupingColumnsn,widerColumn,width);
		//table.ColumnsDefinition(columns =>
		//{
	...
	private static uint printHeaders<T>(TableDescriptor table, List<T> objects, uint currentRow,uint groupingColumns, string widerColumn, int width)
	{
		uint currentColumn = 1;
		table.ColumnsDefinition(columns =>
		{
			foreach (var attribute in objects.First().GetType().GetProperties())
			{
				...
				if (groupingColumns < currentColumn)
				{
					if (attribute.GetCustomAttribute<DisplayNameAttribute>().DisplayName == widerColumn)
						columns.ConstantColumn(width);
					else
						columns.RelativeColumn();
					table.Cell().Row(currentRow).Column(currentColumn - groupingColumns).Text(text =>
					{
						text.Span(attribute.GetCustomAttribute<DisplayNameAttribute>().DisplayName);
						text.DefaultTextStyle(s => s.Bold());
					});
